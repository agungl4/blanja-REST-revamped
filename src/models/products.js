  ...
  sortProduct: (param, param2, limit, offset, page) => {
    return new Promise((resolve, reject) => {
      let queryString = `SELECT m.id, m.product_id, p.product_name, c.category_name, pc.color_name, ps.size_name, pco.condition_name, p.product_price, p.product_desc, p.product_img, m.qty, m.created_at, m.updated_at 
            ...
            JOIN conditions pco ON m.condition_id = pco.id ` + param + ` LIMIT ? OFFSET ?`
          ...
          if (data.length) {
            const newResult = {
              ...
              pageInfo: {
                currentPage: page,
                previousPage:
                  page === 1 ? null : `/products?${param2}page=${page - 1}&limit=${limit}`,
                nextPage: limit != data.length ? null : `/products?${param2}page=${page + 1}&limit=${limit}`,
              },
            }
            resolve(newResult)
          } else {
